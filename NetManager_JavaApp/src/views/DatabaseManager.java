...
public class DatabaseManager {
    ...
    public Map<String, Double> getRevenueByDay(String startDate, String endDate) throws SQLException {
        Map<String, Double> revenueByDay = new HashMap<>();
        String query = "SELECT Ngay, SUM(SoTien) as TotalRevenue " +
                "FROM HOA_DON " +
    ...
    public Map<String, Double> getRevenueByMonth(String startDate, String endDate) throws SQLException {
        Map<String, Double> revenueByMonth = new HashMap<>();
        String query = "SELECT strftime('%Y-%m', Ngay) as Month, SUM(SoTien) as TotalRevenue " +
                "FROM HOA_DON " +
                "WHERE Ngay BETWEEN ? AND ? " +
    ...
    // Lấy số máy sử dụng hôm nay
    public int getMachinesUsedToday() throws SQLException {
        String query = "SELECT COUNT(DISTINCT MaMay) as Count " +
                "FROM HOA_DON " +
                "WHERE Ngay = date('now')";
        try (Statement stmt = connection.createStatement()) {
    ...
    // Lấy số máy sử dụng tuần này
    public int getMachinesUsedThisWeek() throws SQLException {
        String query = "SELECT COUNT(DISTINCT MaMay) as Count " +
                "FROM HOA_DON " +
                "WHERE Ngay BETWEEN date('now', '-6 days') AND date('now')";
        try (Statement stmt = connection.createStatement()) {
            ResultSet rs = stmt.executeQuery(query);
            if (rs.next()) {
                return rs.getInt("Count");
    ...
    public Map<String, Double> getRevenueByStaff() throws SQLException {
        Map<String, Double> revenueByStaff = new HashMap<>();
        String query = "SELECT MaNV, SUM(SoTien) as TotalRevenue " +
                "FROM HOA_DON " +
                "GROUP BY MaNV";
        try (Statement stmt = connection.createStatement()) {
            ResultSet rs = stmt.executeQuery(query);
            while (rs.next()) {
                ...
                double revenue = rs.getDouble("TotalRevenue");
                revenueByStaff.put("NV" + staffId, revenue);
            }
        }
    ...
    public List<String[]> getPaidInvoices() throws SQLException {
        List<String[]> invoices = new ArrayList<>();
        String query = "SELECT MaHD, MaKH, MaNV, MaMay, Ngay, SoTien, HinhThucThanhToan " +
                "FROM HOA_DON " +
                "WHERE SoTien > 0";
        try (Statement stmt = connection.createStatement()) {
            ResultSet rs = stmt.executeQuery(query);
            while (rs.next()) {
                String[] invoice = new String[7];
                invoice[0] = rs.getString("MaHD");
                invoice[1] = rs.getString("MaKH");
                invoice[2] = rs.getString("MaNV");
                invoice[3] = rs.getString("MaMay");
                invoice[4] = rs.getString("Ngay");
                invoice[5] = String.valueOf(rs.getDouble("SoTien"));
                invoice[6] = rs.getString("HinhThucThanhToan");
